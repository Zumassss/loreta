...
import { brl, dataCurta, hoje, id as novoId, mesAtual, mesDe, nomeMes, pct } from '../lib/format'
import {
  caixa as calcCaixa,
  dividirLucro,
  mapaDeCustos,
...
import { CampoDinheiro, Confirmar, Sheet } from '../components/ui'
...
export default function Caixa({ avisar }: { avisar: (t: string) => void }) {
  ...
  const [apagar, setApagar] = useState<{ tipo: 'despesa' | 'retirada'; id: string } | null>(null)
  ...
  const res = resultado(vendas, custos, db.pontos, db.config, periodo === 'mes' ? (vendas.length ? 1 : 0) : undefined)
  ...
  const movimentos = [
    ...despesas.map((d) => ({ tipo: 'despesa' as const, id: d.id, data: d.data, titulo: d.descricao || CATEGORIAS.find((c) => c.id === d.categoria)?.nome || 'Gasto', sub: CATEGORIAS.find((c) => c.id === d.categoria)?.nome ?? '', valor: -d.valor })),
    ...retiradas.map((r) => ({ tipo: 'retirada' as const, id: r.id, data: r.data, titulo: 'Retirada da Julia', sub: r.obs || 'pró-labore', valor: -r.valor })),
  ...
  return (
    <>
      <div className="stack">
        ...
        <section className="hero">
          ...
          <div className="hero__body">
            ...
            <div className="muted" style={{ marginTop: 8 }}>
              {brl(cx.entradas)} de vendas · {brl(cx.saidas)} de gastos · {brl(cx.retiradas)}{' '}
              retirados pela Julia
            </div>
          </div>
        </section>

        <div className="grid-2">
          <button className="btn btn--soft btn--tight" onClick={() => setFormGasto({ id: novoId(), data: hoje(), categoria: 'ingredientes', descricao: '', valor: 0 })}>
            <IcCarrinho style={{ width: 18, height: 18 }} />
            Lançar gasto
          </button>
          <button className="btn btn--peach btn--tight" onClick={() => setFormRetirada({ id: novoId(), data: hoje(), valor: 0, obs: '' })}>
        ...
        <section className="card">
          ...
          <p className="muted" style={{ margin: '4px 0 12px' }}>
            Todo lucro é dividido assim, pra Loreta nunca ficar sem dinheiro pra próxima fornada.
          </p>
          <Potinho
            cor="var(--wine)"
          ...
          {cx.disponivelJulia > cx.saldo && cx.saldo >= 0 && (
            <p className="muted" style={{ marginTop: 8 }}>
              Só tem {brl(cx.saldo)} em caixa agora — tire no máximo isso pra não faltar dinheiro
              pros ingredientes.
            </p>
          )}
        </section>
        ...
        <section className="card">
          ...
          {res.taxas > 0 && <Linha nome="Taxas dos pontos" valor={-res.taxas} />}
          {res.custosFixos > 0 && <Linha nome="Custos fixos" valor={-res.custosFixos} />}
          <div className="divider" />
          <div className="row row--between">
            <span style={{ fontWeight: 800, color: 'var(--wine)' }}>Lucro</span>
            <span className={`money money--lg${res.lucro < 0 ? ' neg' : ' pos'}`}>
              {brl(res.lucro)}
            </span>
          </div>
          {res.faturamento > 0 && (
          ...
          {res.cobertura < 1 && res.unidades > 0 && (
            <p className="muted" style={{ marginTop: 10 }}>
              Atenção: {pct(1 - res.cobertura)} dos brownies vendidos ainda estão sem custo
              cadastrado, então esse lucro está otimista.
            </p>
          )}
        </section>
        ...
        {db.config.custoFixoMensal > 0 && equilibrio.unidades > 0 && (
          <div className="card card--tint">
            ...
            <p style={{ margin: '6px 0 0', fontSize: 14 }}>
              Com {brl(db.config.custoFixoMensal)} de custo fixo por mês, a Loreta precisa vender{' '}
              <b style={{ color: 'var(--wine)' }}>{equilibrio.unidades} brownies</b> (
              {brl(equilibrio.faturamento)}) só pra empatar. Tudo acima disso é lucro.
            </p>
          </div>
        )}
        ...
        {movimentos.length === 0 ? (
          <p className="muted" style={{ textAlign: 'center', padding: '10px 0' }}>
            Nenhuma saída lançada {periodo === 'mes' ? 'neste mês' : 'ainda'}.
          </p>
        ) : (
          <div className="list">
            {movimentos.map((m) => (
              ...
              >
                <div
                  style={{
                    ...
                    height: 36,
                    borderRadius: 12,
                    background: m.tipo === 'retirada' ? 'var(--peach-pale)' : 'var(--alert-bg)',
                    display: 'grid',
                    placeItems: 'center',
                    color: m.tipo === 'retirada' ? '#8a5a1c' : 'var(--alert)',
                    flex: '0 0 auto',
                  }}
                >
                  {m.tipo === 'retirada' ? (
        ...
        {/* guia */}
        <button className="card" style={{ textAlign: 'left' }} onClick={() => setGuia(!guia)}>
          <div className="row" style={{ gap: 10 }}>
            <IcLivro style={{ width: 22, height: 22, color: 'var(--wine)', flex: '0 0 auto' }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800, color: 'var(--wine)', fontSize: 14 }}>
                Guia rápido do dinheiro
              </div>
              <div className="muted">Como a Julia deve lidar com o caixa da Loreta</div>
            </div>
            <IcSetaBaixo
              style={{
                width: 18,
          ...
          {guia && (
            <div className="pop" style={{ marginTop: 14, fontSize: 13.5, lineHeight: 1.6 }}>
              <Regra n={1} titulo="Caixa da Loreta ≠ bolso da Julia">
                O dinheiro que entra é da Loreta. A Julia tira o dela em <b>retiradas</b>, sempre
                registradas aqui. Assim dá pra saber quanto o negócio realmente ganha.
              </Regra>
              <Regra n={2} titulo="Primeiro o custo, depois o lucro">
                De cada venda, a parte que corresponde ao custo dos ingredientes precisa voltar pro
                caixa — é ela que compra a próxima fornada. O que sobra é lucro.
              </Regra>
              <Regra n={3} titulo="Divida o lucro em três potinhos">
                {db.config.splitJulia}% pra Julia, {db.config.splitGiro}% pro giro (comprar mais
                ingredientes e crescer a produção) e {db.config.splitReserva}% pra reserva —
                imprevisto, forma nova, batedeira que queima.
              </Regra>
              <Regra n={4} titulo="Preço mínimo: custo ÷ 0,4">
                Se um brownie custa R$ 4 pra fazer, vender por menos de R$ 10 come a margem. A
                Loreta já sugere o preço na aba Custos.
              </Regra>
              <Regra n={5} titulo="Reserve 1 fornada inteira em caixa">
                Antes de tirar lucro grande, deixe no caixa o valor de pelo menos uma fornada
                completa. É o colchão que impede a produção de parar.
              </Regra>
              <Regra n={6} titulo="Cada ponto tem um preço">
                Anote o faturamento por ponto: em pouco tempo dá pra ver onde vale a pena ir e onde
      ...
      <Sheet
        ...
        rodape={
          <>
            <button className="btn btn--soft btn--tight" onClick={() => setFormGasto(null)}>
              Cancelar
            </button>
            <button className="btn btn--block" onClick={salvarGasto} disabled={!formGasto || formGasto.valor <= 0}>
              Salvar gasto
            </button>
          </>
        }
              ...
              <div className="chips">
                {CATEGORIAS.map((c) => (
                  <button
                    ...
                    onClick={() => setFormGasto({ ...formGasto, categoria: c.id })}
                  >
                    {c.nome}
                  </button>
                ))}
      ...
      <Sheet
        ...
        rodape={
          <>
            <button className="btn btn--soft" onClick={() => setFormRetirada(null)}>
              Cancelar
            </button>
            <button
        ...
        {formRetirada && (
          <div className="stack">
            ...
            {formRetirada.valor > cx.disponivelJulia && (
              <div className="card" style={{ background: 'var(--alert-bg)', borderColor: 'var(--alert)' }}>
                <div style={{ fontWeight: 800, color: 'var(--alert)', fontSize: 13.5 }}>
                  Isso é mais do que o lucro permite
                </div>
                <p className="muted" style={{ margin: '4px 0 0' }}>
                  A cota da Julia é {brl(cx.cotaJulia)} e {brl(cx.retiradas)} já saíram. Tirar mais
                  que isso significa gastar o dinheiro que compra a próxima fornada.
                </p>
              </div>
            )}
            <div className="grid-2">
              ...
              <div className="field">
                ...
                <input
                  ...
                  onChange={(e) => setFormRetirada({ ...formRetirada, obs: e.target.value })}
                />
              </div>
            </div>
            <div className="chips">
              {[0.25, 0.5, 1].map((f) => (
                ...
                >
                  {f === 1 ? 'tudo' : `${f * 100}%`} ({brl(cx.disponivelJulia * f)})
                </button>
              ))}
            </div>
          </div>
        ...
        aoConfirmar={() => {
          ...
          set((d) =>
            apagar.tipo === 'despesa'
              ...
              : { ...d, retiradas: d.retiradas.filter((x) => x.id !== apagar.id) },
          )
...
function Linha({ nome, valor, forte }: { nome: string; valor: number; forte?: boolean }) {
  return (
    <div className="row row--between" style={{ marginBottom: 6 }}>
      <span className={forte ? '' : 'muted'} style={forte ? { fontWeight: 700 } : undefined}>
        {nome}
      </span>
      <span
        style={{
