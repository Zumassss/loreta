...
import { CampoDinheiro, CampoNumero, Confirmar, Sheet } from '../components/ui'
import { IcCheck, IcLapis, IcLixo, IcMais } from '../components/icons'
...
export default function Ajustes({ avisar }: { avisar: (t: string) => void }) {
  ...
  const somaSplit = db.config.splitJulia + db.config.splitGiro + db.config.splitReserva

  const mudarSplit = (chave: 'splitJulia' | 'splitGiro' | 'splitReserva', valor: number) =>
  ...
  return (
    <>
      <div className="stack">
        ...
        <section className="card">
          <p className="muted" style={{ margin: '0 0 14px' }}>
            De cada real de lucro, quanto vai pra cada potinho. A soma precisa dar 100%.
          </p>
          <Slider
            ...
            <div className="row row--between" style={{ marginTop: 4 }}>
              ...
              <button
                ...
                onClick={() =>
                  set((d) => ({
                    ...d,
                    config: { ...d.config, splitJulia: 60, splitGiro: 25, splitReserva: 15 },
                  }))
                }
              >
                voltar ao sugerido
        ...
        <section className="card">
          <div className="field" style={{ marginBottom: 14 }}>
            ...
            <CampoDinheiro
              ...
              aoMudar={(v) => set((d) => ({ ...d, config: { ...d.config, metaMensal: v } }))}
            />
          </div>
          <div className="field" style={{ marginBottom: 14 }}>
            ...
            <CampoDinheiro
              ...
              aoMudar={(v) => set((d) => ({ ...d, config: { ...d.config, custoFixoMensal: v } }))}
            />
            <p className="muted">Gás, energia, internet, transporte fixo — o que sai todo mês.</p>
          </div>
          <div className="field">
            <span className="label">Margem alvo ({pct(db.config.margemAlvo)})</span>
            <input
              type="range"
              min={30}
              max={80}
              ...
              onChange={(e) =>
                set((d) => ({
                  ...d,
                  config: { ...d.config, margemAlvo: Number(e.target.value) / 100 },
                }))
              }
              style={
                {
            ...
            />
            <p className="muted">
              Usada pra sugerir o preço de venda: custo ÷ {(1 - db.config.margemAlvo).toFixed(2)}.
              Pra doce artesanal, 60% é um bom alvo.
            </p>
          </div>
        </section>
        ...
        <div className="list">
          {db.pontos.map((p) => (
            <div key={p.id} className="item">
              <button
                className="chip"
                style={{ padding: 0, width: 30, height: 30, justifyContent: 'center', borderRadius: '50%', background: p.ativo ? 'var(--wine)' : 'var(--cream)', color: p.ativo ? 'var(--cream)' : 'var(--wine-soft)' }}
                onClick={() =>
                  set((d) => ({
                    ...d,
                    pontos: d.pontos.map((x) => (x.id === p.id ? { ...x, ativo: !x.ativo } : x)),
                  }))
                }
              ...
              <div className="item__main">
                <div className="item__title" style={{ opacity: p.ativo ? 1 : 0.5 }}>
                  {p.nome}
                </div>
                {p.taxaPct > 0 && <div className="item__sub">{p.taxaPct}% de taxa do local</div>}
              </div>
              <button className="icon-btn" onClick={() => setPonto(p)} aria-label="Editar">
        ...
        <button
          ...
          onClick={() => setPonto({ id: novoId(), nome: '', ativo: true, taxaPct: 0 })}
        >
        ...
        <div className="list">
          {db.sabores.map((s) => (
            <div key={s.id} className="item">
              <button
                className="chip"
                style={{ padding: 0, width: 30, height: 30, justifyContent: 'center', borderRadius: '50%', background: s.ativo ? 'var(--wine)' : 'var(--cream)', color: s.ativo ? 'var(--cream)' : 'var(--wine-soft)' }}
                onClick={() =>
                  set((d) => ({
                    ...d,
                    sabores: d.sabores.map((x) => (x.id === s.id ? { ...x, ativo: !x.ativo } : x)),
                  }))
                }
              ...
              <div className="item__main">
                <div className="item__title" style={{ opacity: s.ativo ? 1 : 0.5 }}>
                  {s.nome}
                </div>
                {s.precoSugerido ? (
                  <div className="item__sub">vendido a {brl(s.precoSugerido)}</div>
                ) : null}
              </div>
              <button className="icon-btn" onClick={() => setSabor(s)} aria-label="Editar">
                <IcLapis style={{ width: 16, height: 16 }} />
              </button>
            </div>
          ))}
        ...
        <section className="card card--tint">
          <p className="muted" style={{ margin: '0 0 12px' }}>
            Tudo fica guardado no seu próprio celular. Baixe um backup de vez em quando pra não
            perder nada se trocar de aparelho.
          </p>
          <div className="grid-2">
            <button className="btn btn--soft btn--tight" onClick={baixarBackup}>
              Baixar backup
            </button>
            <button className="btn btn--soft btn--tight" onClick={() => setImportando(true)}>
              Restaurar
            </button>
          </div>
          ...
          <div className="row row--between">
            <span className="muted">
              {db.vendas.length} vendas · {db.despesas.length} gastos · {db.ingredientes.length}{' '}
              ingredientes
            </span>
            <button
      ...
      <Sheet
        aberto={!!ponto}
        titulo={ponto && db.pontos.some((p) => p.id === ponto.id) ? 'Editar ponto' : 'Novo ponto'}
            ...
            <>
              {db.pontos.some((p) => p.id === ponto.id) ? (
                <button
                  ...
                  onClick={() => {
                    set((d) => ({ ...d, pontos: d.pontos.filter((p) => p.id !== ponto.id) }))
                    setPonto(null)
                ...
                <button className="btn btn--soft" onClick={() => setPonto(null)}>
                  Cancelar
                </button>
              )}
              <button
        ...
        {ponto && (
          <div className="stack">
            ...
            <div className="field">
              ...
              />
              <p className="muted">
                Se alguém fica com uma parte da venda (comissão), coloque aqui. Se não, deixe zero.
              </p>
            </div>
          </div>
        )}
      ...
      <Sheet
        aberto={!!sabor}
        titulo={sabor && db.sabores.some((s) => s.id === sabor.id) ? 'Editar sabor' : 'Novo sabor'}
            ...
            <>
              {db.sabores.some((s) => s.id === sabor.id) ? (
                <button
                  className="btn btn--soft"
                  onClick={() => {
                    set((d) => ({ ...d, sabores: d.sabores.filter((s) => s.id !== sabor.id) }))
                    setSabor(null)
                ...
                <button className="btn btn--soft" onClick={() => setSabor(null)}>
                  Cancelar
                </button>
              )}
              <button
        ...
        {sabor && (
          <div className="stack">
            ...
            <div className="field">
              ...
              />
              <p className="muted">A receita e o custo você monta na aba Custos.</p>
            </div>
          </div>
        )}
      </Sheet>
      ...
      <Sheet
        ...
        rodape={
          <>
            <button className="btn btn--soft" onClick={() => setImportando(false)}>
              Cancelar
            </button>
            <button
              className="btn btn--block"
